use crate::problem::{Person, PersonId, ProblemDescription, Solution, TableDay, TableDayId, TableId};
...
impl ObjectiveValueCalculator{
    pub fn new(input: &ProblemDescription) -> Self {
        let people_map: AHashMap<PersonId, PersonInner> =
            input.people.iter().map(|p| (p.id, PersonInner::from(p))).collect();
        let table_map: AHashMap<TableDayId, TableDayInner> =
            input.tables.iter().map(|t| (t.id, TableDayInner::from(t))).collect();
        Self {
            people_map,
            table_map,
            relations: input
