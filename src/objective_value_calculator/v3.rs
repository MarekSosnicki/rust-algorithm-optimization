...
pub struct ObjectiveValueCalculator<'a> {
    ...
    relations: HashMap<(PersonId,PersonId), f64>,
}
...
impl<'a> ObjectiveValueCalculator<'a> {
    pub fn new(input: &'a ProblemDescription) -> Self {
        ...
        Self {
            ...
            relations: input
                .people_relations
                .iter()
                .flat_map(|(id1, r)| r.iter().map(|(id2, value)| ((*id1, *id2), *value)))
                .collect(),
        }
    }
    ...
    pub fn table_value(&self, table_day_id: TableDayId, people_ids: &[PersonId]) -> f64 {
        ...
        for (seat, person) in people.iter().enumerate() {
            ...
            result += self
                .relations
                .get(&(person.id.min(people[next_seat].id), person.id.max(people[next_seat].id)))
                .cloned()
                .unwrap_or_default();
