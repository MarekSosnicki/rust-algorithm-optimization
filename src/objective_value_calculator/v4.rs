...
/// Hashmap with better hasher
pub struct ObjectiveValueCalculator<'a> {
    people_map: AHashMap<PersonId, &'a Person>,
    table_map: AHashMap<TableDayId, &'a TableDay>,
...
impl<'a> ObjectiveValueCalculator<'a> {
    pub fn new(input: &'a ProblemDescription) -> Self {
        let people_map: AHashMap<PersonId, &Person> =
            input.people.iter().map(|p| (p.id, p)).collect();
        let table_map: AHashMap<TableDayId, &TableDay> =
